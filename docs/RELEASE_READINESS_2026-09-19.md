...
- Live Scholar testing exposed query contamination: searching `PMC7102549` caused an unrelated result for `PMC7194867` to inherit the queried paper's DOI and a retraction badge. The source extractor now excludes Scholar navigation URLs, whose `q` and `scioq` parameters repeat the search query, and ignores Scholar containers without a result title. The regression test retains the real result's PMCID and direct DOI evidence while rejecting query identifiers. Further live testing found the same contamination in `scholar.googleusercontent.com` cached links; these are now excluded too. Search snippets are no longer treated as the result's identity, and the canonical Scholar title link takes precedence over PDF mirror paths. After the final search-precision reload, the live `PMC7102549` query resolved the overview article to its own DOI `10.1016/j.cmrp.2020.04.001` and no longer assigned the queried retracted DOI to the unrelated cached-PDF results.
...
- Node.js 24.18.1: `npm test` passes all 105 tests; `npm run build` produces Chrome, Edge, Firefox, and Safari targets; `git diff --check` passes.
...
- After the user's reload, live PMC inspection found all 39 references and preserved `bib33` on reference 33, whose inline link is `a[href="#bib33"] > sup`. The popup showed **Formal checks off**, so the absence of formal highlighting at that point does not validate the enabled Crossref flow. After the subsequent reload and the user enabling formal checks, live testing confirmed reference 33 and its inline superscript are both marked `retracted`; references 14 and 19 show correction chips. Clicking reference 33 in the popup scrolled it into the viewport (top 356, bottom 511, viewport height 867), with the retraction background and red inline-citation color. The popup reported 21 checked and 8 not verified, so this is not complete coverage of all bibliography items. No page JavaScript errors were reported.
...
## Search-result precision follow-up

The user's Google query `mdpi paper pubmed` reproduced two sources of noise. The separately installed MDPI Filter added orange `mdpi-potential-reference` boxes to Wikipedia, question modules, social results and related searches. The user disabled that extension; refreshed pages no longer carry those boxes. Notandia itself also classified an entire Google AI answer from links within it and registered empty/navigation containers as results.

Search records now require an engine-specific title destination, exclude Google AI containers, use the innermost result container once, and never simultaneously become bibliography records. Publisher/identifier evidence comes from the destination and explicitly structured metadata, not arbitrary related links or journal words in snippets. Google, Bing and DuckDuckGo redirect wrappers resolve to their destinations. PubMed structured DOI lines and Europe PMC PMID/PMCID fields remain supported. Search-page query URLs cannot become current-article identities. Reused cards do not trust a previous Notandia DOI; destination and metadata mutations schedule rescans, which also remove obsolete highlights.

After the user reloaded the rebuilt extension, live checks found:

- Google `mdpi paper pubmed`: nine result records with sequential IDs, one badge per matching result; PubMed, Reddit and the third-party MDPI discussion page remained unmarked. Related searches were not highlighted. The AI block was not present in that refreshed response; its exclusion and obsolete-style cleanup are covered by HTML tests.
- Bing `mdpi research`: ten result records, no nested highlights; Wikipedia's MDPI article remained unmarked.
- DuckDuckGo `mdpi paper pubmed`: ten result records, no nested highlights; PubMed, PMC and NLM landing pages remained unmarked.
- Scholar `mdpi`: MDPI-hosted publications matched; OUP, ACM, SciELO and the third-party editorial discussing MDPI remained unmarked. No nested highlights.
- PubMed `mdpi`: the inhaler paper using the acronym MDPI remained unmarked; the actual MDPI journal papers matched. Ten result records, no nested highlights.
- Europe PMC `mdpi`: 25 article records, no duplicate bibliography records or nested highlights; action menus were excluded.

All 105 tests pass, including redirect handling, wrong-source/query evidence, duplicate prevention, metadata preservation and rescanning reused cards. Yandex is covered by reduced HTML tests, not a live-site verification in this pass. All four local builds pass.
