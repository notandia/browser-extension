...
test('Google special modules are decomposed into individual evidence units', () => {
  ...
  // People Also Ask / FAQ questions are evaluated independently.
  assert.ok(selector.includes('div.MjjYud .related-question-pair'));

  // AI Overview is source-level: visible source cards and inline source links.
  assert.ok(selector.includes('[data-subtree="mfc"] [role="listitem"]:has(a[href])'));
  assert.ok(selector.includes('[data-subtree="mfc"] span.WBgIic:has(a[href])'));
  assert.ok(selector.includes('[data-subtree="mfc"] mark.HxTRcb:has(a[href])'));
