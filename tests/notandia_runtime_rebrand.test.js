...
test('new general-purpose provider and domain APIs use the Notandia namespace', () => {
  ...
  assert.match(scanner, /window\.NotandiaDomains \|\| window\.MDPIFilterDomains/);
});

test('new DOM records get a Notandia ID while retaining the released compatibility attribute', () => {
  const scanner = source('content/publisher_profile_scanner.js');

  assert.match(scanner, /const REFERENCE_ID_ATTRIBUTE = 'data-notandia-ref-id'/);
  assert.match(scanner, /const LEGACY_REFERENCE_ID_ATTRIBUTE = 'data-mdpi-filter-ref-id'/);
  assert.match(scanner, /element\.setAttribute\(REFERENCE_ID_ATTRIBUTE, id\)/);
  assert.match(scanner, /element\.setAttribute\(LEGACY_REFERENCE_ID_ATTRIBUTE, id\)/);
});
